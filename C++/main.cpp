...
void Export2SVG(int width, int height)
{
    ...
    for(i=0; i<n; i++)
    {
    	...
    	{
        	fprintf(fp,"\n<rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" style=\"fill:#25deed;stroke:black;stroke-width:.30;fill-opacity:0.76;stroke-opacity:0.90\"/>", scale*pixels[2*i+0]-a1+100, scale*pixels[2*i+1]-b1+100, scale, scale);
        }
    } 
...
int main()
{
	...
	b2=y3-y2;
	c2=y3-y1;
    if((x1<x2 && x2<x3) && (y1<=y2 && y2<=y3) && (x3-x2)>=(y3-y2) && (x2-x1)>=(y2-y1) && (x3-x1)>=(y3-y1))
    {
    	size=x3;
